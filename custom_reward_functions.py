...
def detect(prompt: str, log_file_path: str) -> dict:
    ...
    try:
        ...
        return {"score": 0.5}  # Return neutral score on error
    except Exception as e:
...
def ai_detection_reward(prompts: list, completions: list, answer: list, types: list = None) -> list[float | None]:
    """
    Custom reward function that evaluates AI detection score.
    Higher reward for text that appears more human-like (lower AI detection).
    
    Args:
        prompts: List of input prompts
        completions: List of model completions to evaluate
        answer: List of reference answers (not used in this function)
        types: Optional list of types (not used in this function)
    ...
    # trim everything before </think> and trim any whitespace, but if it fails, then just leave the completion as it is
    processed_completions = []
    for completion in completions:
        try:
            processed_completions.append(completion.split('</think>')[1].strip())
        except Exception as e:
            print("Error in AI detection reward: ", e)
            processed_completions.append(completion)

    # Process all completions in parallel
    @loop(enumerate(processed_completions), on_error="collect")
    def process_completion(completion_data):
        i, completion = completion_data

        time.sleep(random.uniform(0, 6))

        try:
            # Get detection score (0-1, where 1 = highly likely AI)
            result = detect(completion, log_file_path)
            detection_score = result.get('score', 0.5)
            
            # Convert to reward: (1 - detection_score) gives reward for human-like text
            # Apply squared transformation to amplify differences: ((1 - score) * 2) ** 2 / 4
            # This maps detection scores: 0->1, 0.5->0.25, 1->0
            reward = (1 - detection_score) ** 1.3
            
            print(f"Completion {i+1}: Detection score: {detection_score:.3f}, Reward: {reward:.3f}")
            return reward
            
        except Exception as e:
            print(f"Error in AI detection reward for completion {i+1}: {e}")
            return None  # Return neutral reward on error
    
    # process_completion is now a list of rewards
    return list(process_completion)